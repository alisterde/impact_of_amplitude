...
# for electrode in ['yellow']:
    ...
    for i in range(0, len(amplitude_vals)):
        ...
        dip_sim = dip_sim*1e6
        plot_name = "$\mathrm{k^{0}} = \mathrm{10^5}\ \mathrm{s^{-1}}$"
        ...
        for kinetic in kinetic_list:


            dip_sim = np.load(os.path.join(input_simulations_base_directory,'k_is_'+str(kinetic)+'_dip_is_'+str(False)+'.npy'))

            dip_sim = dip_sim*1e6
            plot_name = '$\mathrm{k^{0}}$ = ' + str(int(kinetic)) + " $\mathrm{s^{-1}}$"
        ...
        if plot_count==0:
                ...
                if total_sub_plots == 3:
                    ax.legend(loc = "upper center", bbox_to_anchor =[1.6, 1.6], ncol = len(kinetic_list)+2,
                            frameon = False,columnspacing = 0.3, handlelength = 1.0)
                elif total_sub_plots == 7:
                    ax.legend(loc = "upper center", bbox_to_anchor =[3.6 , 1.55], ncol = len(kinetic_list)+2,
                            frameon = False,columnspacing = 0.8, handlelength = 0.9)
                else:
                    ax.legend(loc = "upper center", bbox_to_anchor =[1.6, 1.6], ncol = len(kinetic_list)+2,
                            frameon = False,columnspacing = 0.3, handlelength = 1.0)
    ...
    # plt.show()
    plt.savefig('figure_5_b.png', dpi=500)
