...
# for electrode in ['yellow']:
    ...
    for i in range(0, len(amplitude_vals)):
        ...
        dip_sim = dip_sim = np.load(os.path.join(input_simulations_base_directory,'dispersion_is_'+str(25e-3)+'_dip_is_'+str(False)+'.npy'))
        dip_sim = dip_sim*1e6
        plot_name = "$\mathrm{E^{0}_{\sigma}}$ = "+str(int(0))+' $\mathrm{mV}$'
        ...
        for dispersion in dispersion_list:
            ...
            dip_sim = dip_sim*1e6
            plot_name = "$\mathrm{E^{0}_{\sigma}}$ = "+str(int(dispersion*1E3))+' $\mathrm{mV}$'
        ...
        if plot_count==0:
                ...
                if total_sub_plots == 3:
                    ax.legend(loc = "upper center", bbox_to_anchor =[1.6, 1.6], ncol = len(dispersion_list)+2,
                            frameon = False,columnspacing = 0.3, handlelength = 1.0)
                elif total_sub_plots == 7:
                    ax.legend(loc = "upper center", bbox_to_anchor =[3.75, 1.45], ncol = len(dispersion_list)+2,
    ...
    # plt.show()
    plt.savefig('figure_5_d.png', dpi=500)
